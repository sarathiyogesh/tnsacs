	...
	<table class="table align-middle table-row-dashed fs-6 gy-2 dataTable no-footer" id="kt_ecommerce_products_table">
		...
		<thead>
			...
			<tr class="text-start text-gray-400 fw-bolder fs-7 text-uppercase gs-0">
			   	...
			   	<th class="min-w-100px" tabindex="0" rowspan="1" colspan="1">Amount</th>
		...
		<tbody>
			...
			<tr>
				...
				<td>{{$record->price_label}} ({{date("M", mktime(0, 0, 0, $record->from_month, 10))}} to {{date("M", mktime(0, 0, 0, $record->to_month, 10))}})</td>
				<td><i class="las la-rupee-sign text-dark fs-4"></i> {{number_format($record->amount,2)}}</td>
				<td><a href="javascript:;" class="btn {{ $record->status == 'active'?'btn-success':'btn-danger' }} btn-xs">{!! $record->status !!}</a></td>
				<td>
					<a href="{{url('/courses/create-pricing/'.$record->course_id)}}" class="actionLink"><i class="las la-edit"></i> Edit</a>
				</td>
	        </tr>
	        @endforeach
		</tbody>
