	...
	<table class="table align-middle table-row-dashed fs-6 gy-2 dataTable no-footer" id="kt_ecommerce_products_table">
		...
		<thead>
			...
			<tr class="text-start text-gray-400 fw-bolder fs-7 text-uppercase gs-0">
			   ...
			   <th class="min-w-200px" tabindex="0" rowspan="1" colspan="1">Phone</th>
		...
		<tbody>
			...
			@foreach($records as $record)
				<tr>
					...
					<td>{{ $record->email}}</td>
					<td>{{ $record->mobile }}</td>
					<td>{{ $record->address}} <br/> {{ $record->city}}, {{ State::getname($record->state) }}, India - {{ $record->pincode }}</td>
					<td>{{ Course::getname($record->course) }}</td>
