		...
		<div class="toolbar" id="kt_toolbar">
			...
			<div id="kt_toolbar_container" class="container-fluid d-flex flex-stack">
				...
				<div data-kt-swapper="true" data-kt-swapper-mode="prepend" data-kt-swapper-parent="{default: '#kt_content_container', 'lg': '#kt_toolbar_container'}" class="page-title d-flex align-items-center flex-wrap me-3 mb-5 mb-lg-0">
					<!--begin::Title-->
					<h1 class="d-flex text-dark fw-bolder fs-3 align-items-center my-1">Edit Bulk QR Code</h1>
					<!--end::Title-->
				</div>
				<!--end::Page title-->
				<!--begin::Actions-->
			...
			<div id="kt_content_container" class="container-xxl">
				<div class="card card-flush">
					<div class="card-body">
						...
						<form action="{{url('/bulk-qr-codes/update/'.$record->id)}}" method="POST">
							@csrf
							<div class="row">
								<div class="col-md-6">
									<div class="form-group">
										<label class="required form-label">QR Code</label>
										<input type="text" name="qr_code" class="form-control mb-2" placeholder="QR Code" value="{{old('qr_code', $record->qr_code)}}">
									...
									<div class="form-group">
										...
										<select name="activity_id" aria-label="Select Activity" data-control="select2" data-placeholder="Select an Option" id="activity_id" class="form-select form-select-solid form-select-lg fw-bold">
											...
											@foreach($activities as $activity)
												<option  value="{{ $activity->activity_id }}" @if(old('activity_id', $record->activity_id) == $activity->activity_id) selected="selected" @endif>{{$activity->activity_name}}</option>
											@endforeach
										</select>
									...
									<div class="form-group">
										...
										<select name="item_id" id="item_id" aria-label="Select Activity Item" data-placeholder="Select an Option" class="form-select form-select-solid form-select-lg fw-bold">
											...
											@foreach($items as $item)
												<option value="{{ $item->id }}" @if($item->id == $record->item_id) selected @endif>{{ $item->item_name }}</option>
											@endforeach
										</select>
									...
									<div class="form-group">
										...
										<select name="option_id" id="option_id" aria-label="Select Activity Item" data-placeholder="Select an Option" class="form-select form-select-solid form-select-lg fw-bold">
											...
											@foreach($options as $option)
												<option value="{{ $option->api_tourOptionId }}" @if($option->api_tourOptionId == $record->option_id) selected @endif>{{ $option->api_tourOptionId }}</option>
											@endforeach
										</select>
								...
								<div class="col-md-6">
									<div class="form-group">
										...
										<input type="text" id="expiry_date" name="expiry_date" class="form-control mb-2" placeholder="Expiry Date"  autocomplete="off" value="{{old('expiry_date', $expiry_date)}}">
